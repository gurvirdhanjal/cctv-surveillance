    ...
    <>
      ...
      <div className="p-6 max-w-4xl">
        ...
        <section aria-label="System health" className="mb-8">
          <h2 className="text-[13px] font-semibold uppercase tracking-wider text-text-muted mb-3">
            Health
          </h2>
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-2">API Status</p>
              <StatusBadge ok={health?.status === 'ok'} />
            </div>
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-2">Database</p>
              <StatusBadge ok={ready?.db === 'ok'} />
            </div>
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-2">Redis</p>
              <StatusBadge ok={ready?.redis === 'ok'} />
            </div>
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-2">Version</p>
        ...
        <section aria-label="System metrics">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-1">Head Count</p>
              <p className="text-[28px] font-bold text-text-primary tabular-nums">
                {snapshot?.head_count?.plant_total ?? '—'}
              </p>
            </div>
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-1">Cameras Online</p>
              <p className="text-[28px] font-bold text-text-primary tabular-nums">
                {totalCameras > 0 ? `${onlineCameras}/${totalCameras}` : '—'}
              </p>
            </div>
            <div className="bg-surface-raised rounded border border-border p-4">
              <p className="text-[12px] text-text-muted mb-1">Active Alerts</p>
              <p className="text-[28px] font-bold text-text-primary tabular-nums">
