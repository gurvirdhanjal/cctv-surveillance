...
- [x] 1.1 Create `vms/storage/__init__.py` (empty)
- [x] 1.2 Create `vms/storage/backends.py`:
...
- [x] 1.3 Write `tests/test_storage_backends.py` — unit tests for `generate_key`:
...
- [x] 1.4 RED → run tests → confirm they fail
- [x] 1.5 Implement `generate_key` → GREEN

---

## Task 2 — LocalStorageBackend

- [x] 2.1 Add `LocalStorageBackend` class to `vms/storage/backends.py`:
  ...
  - `exists(key)` → bool
  - `url(key)` → `f"/media/{key}"`
- [x] 2.2 Verify `isinstance(LocalStorageBackend(...), StorageBackend)` — Protocol runtime check
- [x] 2.3 Write tests in `tests/test_storage_backends.py` using `tmp_path` fixture:
...
- [x] 2.4 RED → GREEN → pass

---

## Task 3 — Config additions

- [x] 3.1 Add to `vms/config.py`:
  ```python
  ...
  ```
- [x] 3.2 Write `tests/test_config.py` (add test or new file):
...
- [x] 3.3 RED → GREEN → pass

---

## Task 4 — get_storage() factory

- [x] 4.1 Create `vms/storage/factory.py`:
...
- [x] 4.2 Write tests in `tests/test_storage_factory.py`:
...
- [x] 4.3 RED → GREEN → pass

---

## Task 5 — MinIOStorageBackend

- [x] 5.1 Add `MinIOStorageBackend` class to `vms/storage/backends.py`:
...
- [x] 5.2 Update `factory.py` — handle `storage_backend == "minio"` case
- [x] 5.3 Write `tests/test_storage_minio.py` using `moto` (S3 mock):
...
- [x] 5.4 Add `moto[s3]` to `pyproject.toml` dev dependencies
- [x] 5.5 RED → GREEN → pass
...
- [x] 6.1 Generate migration: `alembic revision -m "storage_key_semantics_thumbnail_and_snapshot_paths"`
- [x] 6.2 Write `upgrade()`:
  ```python
  ...
  """)
  ```
- [x] 6.3 Write `downgrade()` — no-op (data migration is irreversible; keys remain valid in both directions)
- [x] 6.4 Run `alembic upgrade head` locally; confirm no errors
- [x] 6.5 Run `alembic downgrade -1` then `alembic upgrade head` — round-trip clean

---

## Task 7 — Wire GDPR purge to storage backend

- [x] 7.1 Update `vms/api/routes/persons.py`:
...
- [x] 7.2 Write/update `tests/test_api_persons.py`:
...
- [x] 7.3 RED → GREEN → pass
- [x] 7.4 `ruff check vms/ tests/` + `black vms/ tests/` + `mypy vms/` — clean
...
- [x] 8.1 Create `vms/storage/media.py`:
...
- [x] 8.2 Write `tests/test_storage_media.py`:
...
- [x] 8.3 RED → GREEN → pass

---

## Task 9 — Static media mount for local backend

- [x] 9.1 In `vms/api/main.py`, after app creation, mount static files when `storage_backend == "local"`:
  ```python
...
- [x] 9.2 Write test `tests/test_api_main.py`:
...
- [x] 9.3 RED → GREEN → pass

---

## Task 10 — Full test suite verification

- [x] 10.1 `pytest` — all tests pass (target: ≥ 176 existing + new storage tests)
- [x] 10.2 `pytest --cov=vms/storage --cov-report=term-missing` — ≥ 90% coverage on `vms/storage/`
- [x] 10.3 `ruff check vms/ tests/` — clean
- [x] 10.4 `black vms/ tests/` — no changes
- [x] 10.5 `mypy vms/` — clean (no new errors)
- [x] 10.6 Update this plan: mark all tasks `[x]`, set `**Status: COMPLETE**`
- [x] 10.7 Update `CLAUDE.md §3` — note Storage Abstraction complete with commit hash
