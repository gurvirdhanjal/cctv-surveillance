...
| `docs/superpowers/specs/2026-07-08-vms-model-stack-and-analytics-readiness.md` | **Model stack & analytics readiness (Approved 2026-07-08).** Authoritative model inventory (all 7 models: weights, preprocessing, thresholds, latency), serving architecture, frame-to-person binding, day-one requirement traceability, gap register. Defines the person timeline API (§9.2) and homography calibration + floor read APIs (§8.2) — both planned into Phase 4P. Records NVDEC activation (§4.4 → Phase 6d plan) |
...
**Last major milestone:** Phase 4M Interactive Surfaces — COMPLETE (795 tests, 2026-07-08). §R Workspace prefs (Zustand persist: pinnedCameraIds, hiddenAlertTypes, cmdkRecents, treeExpansion, treeOrder); §D DataTable (TanStack Table v8 + react-virtuoso, sortable + column-reorder via dnd-kit, row-select, sticky header, pinned-rows, CSV export); §E WorkspaceShell (react-resizable-panels 3-column live layout, panel-size persist); §I resizable TimeScrubber panels; §F+§J PremiumCameraCard + FloatingActionBar (Framer Motion hover-lift, glass overlay, 3-button quick-actions); §Q hierarchical CameraTree (5-level Site→Building→Floor→Zone→Camera, dnd-kit sortable zones, Framer Motion collapse, Virtuoso for >200 cameras, Alembic migration for site_name/building_name/floor_name); §P CommandPalette upgrade (6 groups + Recents, glass-cmdk z-[60], cmdk v1); §V.2 ECharts migration (recharts removed, EChartsWrapper lazy + useChartTheme, 3 charts rewritten); Toast/ToastProvider cleanup; SkeletonTimeline/SkeletonCameraTree/SkeletonChart composites. Plan: `docs/superpowers/plans/2026-07-07-vms-phase4m-interactive-surfaces.md`.

**Previous milestone:** Phase 4L Enterprise UX Polish — COMPLETE (722 tests, 2026-07-07). Design system hardened end-to-end: §G tokens (radius/duration/shadow/state), §H motion (EASE + 8 presets), §C elevation z-index ladder, §B icon registry (36-name), §A primitives (Box/Stack/Cluster/Surface), §K CameraStatusBadge (13-state), §L skeleton composites (KpiCard/CameraCard/TableRow/Avatar), §M glass/blur (4 permitted surfaces + reduced-transparency), §N nav compliance, §O micro-interactions (Button 0.97 press, Checkbox spring 500/30, Toggle spring 400/25), §S ActionBar + reference adoption, §T visual-restraint CI gate (6 brass focus rings fixed), §V.1 Sonner VmsToaster. Plan: `docs/superpowers/plans/2026-07-07-vms-phase4l-enterprise-ux-polish.md`.

**Previous milestone:** Phase 4K Live View Dark Operator Console — COMPLETE (559 tests, 2026-07-07). Premium dark `/live` operator console: three-column workstation (320px CameraTree | 1fr FocusedCamera | 380px AlertSidebar), forced `data-theme="dark"` scope, dark surfaces (`#0a0e1a`/`#111827`/`#1a2234`). New: `CameraTree` (paginated 12-tile grid, search, alarming pulse), `AlarmCard` + `useCountdown` (SLA bar, severity border), `BoundingBoxOverlay` (5fps rAF throttle, known/unknown/followed colors), `useHlsStream`, `GridLayoutSelector`, `useLiveShortcuts` (A/R/B/E/N/F/?), `ShortcutLegend`, `ClipExportDialog` (RHF+Zod, POST /api/forensic/export), `OfflineReconnectBanner` (Framer Motion slide), `SystemStatusStrip`, `TopBar` (dark, Cmd+K, GPU bar, head count). Plan: `docs/superpowers/plans/2026-07-07-vms-phase4k-live-view-operator-console.md`.

**Phases 4N/4O/4P — Frontend Premium Polish & Data Completeness:** Spec approved `docs/superpowers/specs/2026-07-08-vms-frontend-premium-polish.md` (audit-backed; open questions resolved 2026-07-08). Three plans written, **NOT STARTED — awaiting user review**: `2026-07-08-vms-phase4n-design-language.md` (token/typography consolidation, hex+palette CI ban, minimal AppSidebar replacing shadcn, AlertCard/AlarmCard merge, CameraRow tree redesign, shell unification), `2026-07-08-vms-phase4o-ui-states-premium.md` (UI-state doctrine matrix, useNow/formatters, chart token ramp, command-palette actions, login brand moment), `2026-07-08-vms-phase4p-data-completeness.md` (7 new endpoint groups: persons detail, analytics KPI + head-count rollup, system metrics via scheduler→Redis, clip export 202-queued, users CRUD, bookmarks; **extended 2026-07-08** per the model-stack spec: Task 1b person timeline `GET /api/persons/{id}/timeline`, Tasks 8b/8c homography calibration API + `floor_plans` + heatmap/floor read APIs, Task 14b calibrator/heatmap/timeline frontend wiring; 6 migrations; orphan-endpoint wiring; dead-call removal). Execution order: 4N → 4O; 4P backend tasks parallelizable. **4P backend Tasks 1–8c COMPLETE (2026-07-09, 931 tests, commits `172863ea`..`b8eca66a`)**: persons detail + timeline, camera_status_events, KPI + head-count rollup, system metrics + WS relay, clip export 202-queued, users CRUD, bookmarks, homography calibration + floor read APIs. Frontend wiring (Tasks 9–15) awaits 4N; Task 16 closes the phase.

**Phase 5 — Enterprise Interaction Guidelines:** Spec written `docs/superpowers/specs/2026-07-06-vms-enterprise-interaction-guidelines.md`. **NOT STARTED — plan not yet written.** Covers: operator alarm workflow, keyboard shortcuts, multi-monitor layouts, live video controls, SLA/escalation model, camera health score, auditability UI, error handling, offline/reconnect, performance budgets. Must be planned before implementation.
...
**Phase 6d — NVDEC GPU decode:** Plan written `docs/superpowers/plans/2026-07-08-vms-phase6d-nvdec-gpu-decode.md`. **NOT STARTED — awaiting user review.** §6.3 build trigger met: user confirmed 2026-07-08 that CPU stream decode is the bottleneck on live multi-camera runs. FFmpeg `h264_cuvid`/`hevc_cuvid` subprocess behind a `DecodeBackend` protocol in `vms/ingestion/decoder.py`, NVDEC session ledger with explicit CPU fallback (never silent), decode-parity + detection-parity integration gates, capacity probe. Task 0 records the quantified CPU baseline before any code.

**Defer:** INT8 quantization (only needed past 52 cameras).
...
- **`adaface_min_sim=0.72` needs re-calibration** — threshold set for old IR50/MS1MV2 unaligned embeddings. IR101/WebFace12M + affine alignment shifts the embedding distribution. Re-run calibration on real footage; use `/advisor` before setting a new value (§0.5 mandatory trigger).
